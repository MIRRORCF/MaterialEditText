...
class MaterialMultiAutoCompleteTextView : AppCompatMultiAutoCompleteTextView {
    ...
    private val buttonsCount: Int
        private get() = if (isShowClearButton()) 1 else 0
     ...
     */
    fun addValidator(validator: METValidator): MaterialMultiAutoCompleteTextView {
        if (validators == null) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (hasFocus() && showClearButton && !TextUtils.isEmpty(text) && isEnabled) {
            ...
            val iconTop = lineStartY + bottomSpacing - iconOuterHeight + (iconOuterHeight - clearButtonBitmap.height) / 2
        ...
        if (floatingLabelEnabled && !TextUtils.isEmpty(floatingLabelText)) {
            ...
            floatingLabelStartX = if (gravity and Gravity.RIGHT == Gravity.RIGHT || isRTL) {
            ...
            val floatingLabelStartY = (innerPaddingTop + floatingLabelTextSize + floatingLabelPadding - distance * (if (floatingLabelAlwaysShown) 1f else floatingLabelFraction) + scrollY) as Int

            // calculate the alpha
            val alpha = ((if (floatingLabelAlwaysShown) 1f else floatingLabelFraction) * 0xff * (0.74f * focusFraction * (if (isEnabled) 1f else 0f) + 0.26f) * if (floatingLabelTextColor != -1) 1f else Color.alpha(floatingLabelTextColor) / 256f) as Int
            textPaint.alpha = alpha

            // draw the floating label
            canvas.drawText(floatingLabelText.toString(), floatingLabelStartX.toFloat(), floatingLabelStartY.toFloat(), textPaint)
        }
        ...
        if (hasFocus() && singleLineEllipsis && scrollX != 0) {
            ...
            val ellipsisStartX: Int
            ellipsisStartX = if (isRTL) {
                endX
            } else {
                startX
            }
